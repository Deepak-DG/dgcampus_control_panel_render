...
import BulkUploadSlot from "@/app/lib/slots/BulkUploadSlot";
...
const ManageSlots: React.FC = () => {
  ...
  //DELETE action
  const openDeleteConfirmModal = (row: Slot) => {
  ...
  const table = useMaterialReactTable({
    ...
    enableEditing: true,
    onEditingRowSave: handleEditSlot,
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <>
        ...
        <BulkUploadSlot getSlots={getSlots} />
      </>
    ),
  });

  return <div>{slots.length >= 0 && <MaterialReactTable table={table} />}</div>;
};
