...
const SelectedSubscriptions: React.FC = () => {
  ...
  const getSelectedSubscriptions = async () => {
    ...
    await axiosInstance
      .get(`pack/selected_subscriptions?${query}`)
      .then((response) => {
        console.log(response.data);
        setSelectedSubscriptions(response.data.results);
  ...
  const handleCreateSelectedSubscription = async ({
    values,
    table,
  }: {
    values: SelectedSubscription;
    table: MRT_TableInstance<SelectedSubscription>;
  }) => {
    axiosInstance
      .post(`pack/selected_subscriptions/`, {
        user: values.user,
  ...
  const handleEditSelectedSubscription = async ({
    values,
    table,
  }: {
    values: SelectedSubscription;
    ...
    axiosInstance
      .patch(`pack/selected_subscriptions/${values.id}/`, {
        user: values.user,
        service: values.service,
        duration_months: values.duration_months,
        pickups_per_month: values.pickups_per_month,
  ...
  const columns = useMemo<MRT_ColumnDef<SelectedSubscription>[]>(
    () => [
      {
  ...
  //DELETE action
  const openDeleteConfirmModal = (row: SelectedSubscription) => {
    if (window.confirm("Are you sure you want to delete this selected subscription?")) {
      console.log(row.id);
      axiosInstance
        .delete(`pack/selected_subscriptions/${row.id}/`)
        .then((response) => {
  ...
  const table = useMaterialReactTable({
    ...
    onCreatingRowSave: handleCreateSelectedSubscription,
    enableEditing: true,
    onEditingRowSave: handleEditSelectedSubscription,
    //optionally customize modal content
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Create New Selected Subscription</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Selected Subscription</DialogTitle>
        <DialogContent
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Button variant="contained" onClick={() => table.setCreatingRow(true)}>
        Create New Selected Subscription
      </Button>
    ),
  });

  return (
    <div>{selectedSubscriptions.length >= 0 && <MaterialReactTable table={table} />}</div>
  );
};
